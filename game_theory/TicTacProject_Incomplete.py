...
import Tic_Tac_Toe as game_helpers
from Tic_Tac_Toe import checkWinPos
...
aiSkill = 3
#Change to 0 for no AI, Change to 1 for Easy, Change to 2 for Medium, 3 for Hard
...
"""
Code below provided with programming assignment
"""
def getPlace():
    global place
    return place
...
def AI(aiSkill,place,turn,AIturn):
    ...
    if aiSkill == 1: #Easy
        if place[4] == 0:
            ...
            midRightPress()
    if aiSkill == 2: #Medium
        # Here is where we are placing the Utility Based Functioin
        # Persist: state is being evaluated by taking the game board and checking each row, column and diagonal
        #          model is where an evaluation function will be used
        #          utility-function is the evaluation function Eval(s,d) = 3*X2(s)+X1(s)-(3*O2(s)+O1(s))
        #          plan is to determine the best row, column or diagonal to place a new piece ont he board
        #          action is to pass the location on the board to place my X on
        F = checkWinPos(place)
        if F != None:
            if F == 0:
                topLeftPress()
    ...
    if aiSkill == 3: #Hard
        ...
        F = game_helpers.decision_maker(G,0,0)
        if F == 0:
            topLeftPress()
        if F == 1:
            midLeftPress()
